...
- **EVM** — 无 calldata 的原生转账（正 value）及标准 ERC-20/721/1155 转账的最终收款方

`approve` 及带 calldata 的合约调用不受收款方白名单限制，包括向路由合约携带原生币的 swap。提示词中的别名列表仅作辅助，且会省略无别名地址；最终是否允许必须由 build/sign 门控读取完整白名单决定。
