export default {
    ...
    field: {
        ...
        agentMarketURL: 'Agent Market URL',
		agentValidatorURL: 'Agent Validator URL',
        addressType: 'Address Type',
    ...
    ph: {
        ...
        agentMarketURL: 'Semantic agent search service; leave empty for https://agent-market-devnet.svpstars.com',
		agentValidatorURL: 'Leave empty for https://agent-validator-devnet.svpstars.com',
    ...
    hint: {
        ...
        whitelist: 'When configured, Cosmos sends, native EVM transfers, and standard token transfers may only go to whitelisted addresses. Approvals are not recipient-whitelisted, but still require local confirmation.',
        ...
        remoteMCPDisabled: 'When on, the assistant does not connect to the remote MCP service at all: no transaction building, broadcasting, or market data. It keeps local signing plus on-chain agent discovery. Useful for working against the chain alone.',
    ...
    runs: {
        ...
        loggingOff: 'New runs are not being recorded. Existing traces remain; turn the setting back on to log more.',
        empty: 'No run traces yet. Send a message in Assistant, then come back to inspect tools and outcomes.',
        emptyFilter: 'No runs match this filter',
        untitled: '(empty)',
        when: 'When',
        model: 'Model',
        tokens: 'Tokens',
        ...
        rounds: '{n} round(s)',
        promptHash: 'System prompt hash',
        skills: 'Injected skills',
        session: 'Conversation',
        openSession: 'Open conversation',
        sessionGone: 'That conversation was deleted',
        userMessage: 'User message',
        error: 'Error',
        answer: 'Answer',
        txHashes: 'Transaction hashes',
        txRecheck: 'Recheck on chain',
        intents: 'Intent check',
        intentStatus: {
            matched: 'Events match',
            mismatch: 'Does not match events',
            included: 'Included (no event compare)',
            unobserved: 'Not observed yet',
            skipped: 'Chain RPC not configured',
        },
        txStatus: {
            confirmed: 'On-chain',
        ...
        status: {
            ...
            count: '{n} recent run(s)',
            loadFailed: 'Failed to load runs: {err}',
            pathCopied: 'Log path copied',
            ...
            recheckFailed: 'On-chain recheck failed: {err}',
            rechecked: 'On-chain status updated',
        },
    },
    skill: {
    ...
    about: {
        ...
        version: 'Version {v}',
        lead: 'A local-key on-chain assistant for svpchain (Cosmos + EVM). Your private key stays on this machine. The assistant can find remote agents by task through the Agent Market service and message them over A2A, and it runs the full remote-build → local-sign → remote-broadcast path itself.',
        trust: {
            ...
            assistant: 'Orchestrates tool calls. It cannot sign for you, and it cannot override the whitelist or the confirmation dialog.',
        },
        canDo: {
            title: 'What you can do',
            orchestrateTitle: 'Find agents.',
            orchestrate:
                'Ask the assistant to find a remote agent by describing the task: it searches the Agent Market service and reports each agent\'s endpoint, capabilities and advertised pricing, then can message one over A2A. Those results come from the search service, and the message carries no authority over your account — anything that moves funds still goes through local signing.',
            ...
            security: 'Allow-list transfer recipients. The assistant refuses transfers to addresses outside the whitelist.',
            mcpTitle: 'MCP.',
        ...
        safety: {
            title: 'Safety',
            whitelist:
                'The whitelist runs before the confirmation dialog: confirming cannot override a transfer whitelist refusal. Approvals are not recipient-whitelisted, but still require confirmation. Empty whitelist in the assistant = refuse all transfers; empty whitelist in the standalone signer = unrestricted (backward compatible).',
            confirm: 'Every local signature (except the MCP auth challenge) needs an explicit approve in a dialog. Decline, timeout, or no one present all deny.',
        ...
        start: {
            ...
            steps: [
                ...
                'In Settings, enter an LLM API key, pick a network, and set an Agent Market URL if needed.',
                'Add at least one whitelist entry in Security, or the assistant will refuse transfers.',
                'Pick a network in the sidebar, then talk in Assistant.',
            ],
        },
    },
    status: {
