...
- **Empty grants deny.** Actions and subaccounts must be explicit. The
  ...
  `evm.native_transfer`, `settlement.record_spend` (added automatically with `service_budget`),
  and the read-only `query.account`.
- **Value-committing actions need a budget.** The chain prices an order and
  checks it against the credential's own budget, so `clob.place_order` or
  `evm.native_transfer` without a `budget` is refused. Size the budget for
  this order (its notional, in the market's quote denom) or transfer (its
  exact `asvp` value), not for the delegation's whole allowance.
