...
- `internal/agent/` — the LLM tool-calling loop (`runner.go` `Run`, `dispatchTool`). Holds a remote MCP client + in-process `LocalSigner` (`local.go`). Local-only tools (`signer_whoami`, `evm_to_bech32`, `http_fetch`, `x402_*`, `a2a_send_message`) live here and are **not** exposed by the stdio server.
  - `internal/agent/llm/` — ChatModel adapter. `Client` owns retries and latency; `openaiModel` / `anthropicModel` implement `llm.Model` (stream + tool calls only). The runner never talks HTTP to a provider and **never** lets the model execute tools. Inject a fake via `NewClientWithModel` in tests.
  ...
  - `internal/agent/writepath/` — per-run state machine that enforces remote `build_*` → local `sign_*` → remote `broadcast_*`. Signing a payload that was not built this run, broadcasting without a matching signature, or mutating `signed_tx` stops the run. Cosmos and EVM are independent lanes. `sign_typed_data` / x402 are outside this graph.
  - `middleware.go` — handler chain: observe (runlog today; OTel via `ToolObserver`) → whitelist gate → write-path graph → whoami cache → protocol mux (`handlers.go`). HITL sits inside the local-signer handler so a gate/graph rejection never opens a dialog. The mux's remote catch-all is last so a local `sign_*` never reaches the remote MCP.
...
1. **Assistant pre-flight gate** (`internal/agent/guard/gate.go`) — checks the recipient/spender from tool *arguments* before forwarding a `build_*` call. **Mandatory: an empty whitelist refuses ALL transfers/approvals.** It also gates `sign_evm_transaction` on the decoded transaction as defense in depth (the write-path graph additionally requires a matching `build_*` from this run before that tool may run).
...
- **Trust-boundary files need extra care.** Changes under `internal/signer/`, `internal/payload/`, `internal/whitelist/`, `internal/evmcall/`, `internal/mcp/`, `internal/agent/guard/`, `internal/agent/hitl/`, `internal/agent/writepath/`, or `internal/agent/chainid/` touch the security boundary. Preserve the chain-id binding and `signer_address` cross-checks, the `svpchain-mcp-auth-v1:` challenge prefix guard, the two-layer whitelist semantics (gate: empty = refuse all; signer: empty = unrestricted), and the write-path order (build → sign → broadcast, `signed_tx` verbatim). HITL must not override a whitelist rejection. Add or extend tests in the matching `_test.go`.
