...
func TestDispatchToolUsesCachedWhoami(t *testing.T) {
	...
	out, err := dispatchTool(context.Background(), "svp-2517-1", nil, nil, nil, "signer_whoami", nil, mem)
	require.NoError(t, err)
	require.JSONEq(t, mem.SignerWhoami, out)

	out, err = dispatchTool(context.Background(), "svp-2517-1", nil, nil, nil, "whoami", nil, mem)
	require.NoError(t, err)
