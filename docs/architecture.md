...
```
            ┌──────────────────────────────┐
            │  LLM (OpenAI-compat / Anthropic) │   streaming tool-calling loop
            └───────────────┬──────────────┘
                            │
              ┌─────────────┴──────────────┐
...
For **multi-agent** workflows, the assistant finds agents with `search_agents` (the Agent Market service) and talks to
them with the uncredentialed plain-text `a2a_send_message` — see [Agent-to-Agent (A2A)](a2a.md). This agent is an A2A
client only; it never listens as a service.
