...
助手将转账/授权的 `build_*` 调用转发到远程 MCP 前，直接从工具参数读取收款方/spender 并与白名单比对。
**此处白名单为强制：未配置白名单时，一切转账/授权均被拒绝**，并提示先在「安全」标签添加 —— 不会构建、签名或广播。已有白名单时，非白名单地址同样拒绝，提示
...
签名层不解码合约调用与零 value EVM 交易，此类场景由上层预检拦截。独立 `svpchain-mcp` signer 读取同一 `prefs.json`
，但不施加助手的强制白名单规则。
