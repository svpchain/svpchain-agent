...
// broadcasting without a matching signature, or mutating signed_tx.
...
type lane struct {
	...
	signedTx string // canonical JSON of signed_tx from sign_*
	signed   bool
}
...
func (t *Tracker) beforeBroadcast(family Family, name string, args map[string]any) error {
	signed, err := canonicalSignedTx(args)
	if err != nil {
...
func canonicalSignedTx(args map[string]any) (string, error) {
	if args == nil {
		return "", fmt.Errorf("missing signed_tx")
	}
	...
	// Some callers flatten SignedTx / SignedEvmTx onto the argument object.
	if _, ok := args["tx_raw_bytes_b64"]; ok {
		return canonical(args)
	}
	if _, ok := args["raw_tx_hex"]; ok {
		return canonical(args)
	}
	return "", fmt.Errorf("missing signed_tx")
}
